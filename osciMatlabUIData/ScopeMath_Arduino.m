function varargout = ScopeMath_Arduino(varargin)
...
%
% Example usage:
% ScopeMath_Arduino %launch program with defaults
% ScopeMath_Arduino('copyBitmap') %copy image to clipboard
% ScopeMath_Arduino('saveBitmap',{'myPicture.png'}); %save image to disk
...
print (v.hFigure, '-r600', '-dpng', filename); %<- save as 600dpi, '-noui'
%end saveBitmap()

% --- Copy screenshot to clipboard
function copyBitmap(v)
%simpleUI('copyBitmap')
editmenufcn(v.hFigure,'EditCopyFigure');
%end copyBitmap()


  %%---------------------------------------------------
   function getDataFromDeviceType(hObject, eventdata)
       h = findall(0,'tag',mfilename); %input hObject is for timer, not figure
       v = guidata(h);
       switch v.deviceType
        case 1
            [newData] = getDataFromDeviceType1(h);
           otherwise
            [newData] = getDataFromDeviceType0(h);
     ...
     if ~isempty(v.gSaveDataBaseFilename)
        ...
        if (v.gSaveNumber > v.gSaveEveryNRefreshes)  %flush unsaved data to disk
            guidata(v.hFigure,v);% <- REQUIRED to save changed settings
            flushSaveDataSub(h);
        end; %if SaveNumber
     end %if SaveData
...
guidata(v.hFigure,v);% <- REQUIRED to save changed settings
  ...
  %%---------------------------------------------------
    function [newSamples0] = getDataFromDeviceType0(hObject)       
  ...
  %%---------------------------------------------------
    function [newSamples1] = getDataFromDeviceType1(hObject)        
        v = guidata(hObject);
        ...
        [newSamples1, v.rawData] = serDecodeSub(hObject, v.rawData);
        guidata(v.hFigure,v);% <- REQUIRED to save changed settings
...
%Powerpoint wireless presenter devices generat pageup/pagedown and hide
 ...
 function [vFig] = makeGUI()
    ...
    v.serDefaultIndex = 1; % 0 for auto-select, 1 for simulated data, 2 for Arduino on port 1, 3 for Arduino on port 3, etc
    ...
    v.hFigure = figure('deleteFcn', {@figureCloseCallback},'name',mfilename,'Tag', mfilename,'units','pixels','position',[40, 40, 1024, 512],'KeyPressFcn', @figureKeyPress);
   ...
   vFig = v.hFigure;
   guidata(v.hFigure,v);%store settings

   %end %makeGUI()
  
  %%---------------------------------------------------
    function[OK] =  startDeviceType(hObject)
    v = guidata(hObject);
    v.gUnsavedData = [];
    v.rawData = []; %raw data from device - data that still needs to be decoded into discrete samples
  ...
  %%---------------------------------------------------
   function [OK] = startDeviceType0(hObject) %DeviceType 0 = simulated data
       v = guidata(hObject); 
   ...
   %%---------------------------------------------------
   function [OK] = startDeviceType1 (hObject, DeviceName) %DeviceType 1 = Arduino on serial port
  ...
  %%---------------------------------------------------
   function stopDeviceType(hObject)
    v = guidata(hObject);
    switch v.deviceType
     case 1
        stopDeviceType1(hObject);
     otherwise
         stopDeviceType0(hObject);
    end %switch deviceType 
    flushSaveDataSub(hObject); %save any residual data to disk
    fcloseSerialSub(hObject);
   %end %stopDeviceType()

   function stopDeviceType0(hObject)
         disp('Simulated acquisition halted');
   %end %stopDeviceType0()

  %%---------------------------------------------------
   function stopDeviceType1(hObject)
       v = guidata(hObject); 
  ...
  %%---------------------------------------------------
   function flushSaveDataSub(hObject)
    v = guidata(hObject);
    if isempty(v.gSaveDataBaseFilename) 
        return; 
    end
...
%%---------------------------------------------------   
  ...
  %%---------------------------------------------------   
   function figureCloseCallback(hObject, eventdata)
       cleanupObjects(hObject);
   %end %figureCloseCallback()

  %%---------------------------------------------------   
   function cleanupObjects(hObject)
       v = guidata(hObject);
  ...
  %%---------------------------------------------------   
   function fcloseSerialSub(hObject)
       v = guidata(hObject);
       if ~isnumeric(v.serialObj) &&  isvalid(v.serialObj) 
        if (v.serialObj.BytesAvailable > 0)
  ...
  %%---------------------------------------------------
   function [theSamples, rawResidual] =serDecodeSub(hObject, rawData)
       v = guidata(hObject); 
       theSamples =[];
        len = length(rawData);
        samples = 0;
