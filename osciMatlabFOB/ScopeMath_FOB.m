function ScopeMath_FOB
%ScopeMath_Arduino a simple oscilloscope for flock of birds 
%
...
global v
v.hzChoices = [100]; %acquisition rate: maximum depends on hardware 
v.chChoices = [1 2 3]; %number of channels: maximum depends on hardware (Leonardo supports 6, Teensy3 supports 14)
...
v.chDefaultIndex = 3; %e.g. if 2, 2nd option of ChChoices is the default
...
%Powerpoint wireless presenter devices generat pageup/pagedown and hide
  ...
  %%---------------------------------------------------
   ...
   function [OK] = startDeviceType1 (DeviceName) %DeviceType 1 = Arduino on serial port
       ...
       global v;
       OK = true; % assume failure to connect
       if isempty(DeviceName)
            fprintf('FOB Data: Recording %d channels at %d Hz\n',v.gOscChannels-1,v.gOscHz);
        else
            fprintf('FOB Data: Recording %d channels at %d Hz attached to port "%s"\n',v.gOscChannels-1,v.gOscHz,DeviceName);
        end;
        fcloseSerialSub();
        v.hAxisRaw(3) = -32768;  v.hAxisRaw(4) = 32768; %16-bit acquisition data ranges 0..65535
        ...
        %pause(sec)
        if (strfind(v.serialObj.Status,'open')),fwrite(v.serialObj,[uint8('B')]); end; %B Point command - p 121 -  request single measure, stop streaming
        
        pause(0.450); %wait at least 300ms before autoconfig
        fwrite(v.serialObj,[241,80,50,1]);%address master: change : auto config : 1 bird
        pause(0.670); %wait at least 300ms after autoconfig
        %fwrite(v.serialObj,[80,7,1,86]);%change measurement rate 86.1 Hz
        %pause(0.670);
        fwrite(v.serialObj,[uint8('V')]); %send position data
        
        v.rawData = []; %clear raw data from device
        
        %END ERROR DETECTION SETION
        fwrite(v.serialObj,[uint8('@')]); %@ STREAM mode - p 138 - Data records will continue to be sent until the host sends the POINT command       
  ...
  %%---------------------------------------------------
   function stopDeviceType1()
       global v; 
       disp('FOB acquisition halted');
       if ~isnumeric(v.serialObj) &&  isvalid(v.serialObj) 
           fwrite(v.serialObj,[uint8('B')]); %B Point command - p 121 -  request single measure, stop stream
       end;
   %end %stopDeviceType1()
...
%%---------------------------------------------------   
  ...
  %%---------------------------------------------------
   function [theSamples, rawResidual] =serDecodeSub(rawData)
       ...
       theSamples =[];
        len = length(rawData);
        samples = 0;
        packetBytes = 6; %16-bits data per channel plus 4 bytes header
        i = 1;
        while (i <= (len-(packetBytes-1)))
            if (rawData(i) > 127) && (rawData(i+2) < 128) && (rawData(i+4) < 128)
                samples = samples + 1;
                for j=1:(v.gOscChannels)
                    lo = rawData(i+(j*2)-2);
                    hi = bitand(rawData(i+(j*2)-1),127); 
                    % s = int16(32000);
                    % b = typecast(s,'uint8'); %b(1)=lowByte, b(2)=highByte
                    % hi = bitshift(b(2),-1);
                    % lo = bitshift(bitand(b(2),1),+6)+ bitshift(b(1),-2);
                    dx = bitshift(double(hi),9) + bitshift(double(lo),2) ;
                    ux = uint16(dx);
                    sx = typecast(ux,'int16');

                 theSamples(j,samples) =  sx ;
                end;
                i = i + packetBytes;
                OKpos = i - 1;
            else
  ...
  %%---------------------------------------------------
   function ser=serDeviceIndexSub(DeviceName)
        ...
        ser = serial(DeviceName,'InputBufferSize',16384,'BaudRate',57600);%most reliable to set correct BPS - helps if device is already running at high speeds
        fopen(ser);
